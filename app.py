...
.shine-title h2 {
    ...
    color: #FFD7A8;
    margin: -0.3rem 0 0 0;
    text-shadow: 0 0 20px rgba(255,215,168,0.4);
}
.shine-title p {
    font-size: 0.75rem;
    color: #FFECD2AA;
...
[data-testid="stTextInput"] input {
    ...
    border: 1px solid rgba(255,236,210,0.25) !important;
...
.stButton > button:hover {
    background: rgba(255,215,168,0.35) !important;
...
[data-testid="stFileUploader"] span { color: #FFECD2AA !important; }
...
/* Checkbox */
[data-testid="stCheckbox"] label span { color: #FFECD2CC !important; font-size: 0.8rem !important; }
</style>
""", unsafe_allow_html=True)

# ── Helper functions ────────────────────────────────────────────────
def load_all():
    """Load all records from Supabase, ordered by name."""
    res = sb.table("checkin").select("*").order("nome").execute()
    return res.data or []

def get_counts():
    data = load_all()
    total = len(data)
    arrived = sum(1 for d in data if d.get("checked_in"))
    return total, arrived, total - arrived

def toggle_checkin(record_id, current_val):
    sb.table("checkin").update({"checked_in": not current_val}).eq("id", record_id).execute()

def upload_names(df):
    """Clear table and insert new names."""
    sb.table("checkin").delete().neq("id", 0).execute()
    records = [{"nome": str(n).strip(), "checked_in": False} for n in df.iloc[:, 0] if str(n).strip()]
    # Insert in batches of 100
    for i in range(0, len(records), 100):
        sb.table("checkin").insert(records[i:i+100]).execute()

def has_any_checkin():
    """Check if anyone has checked in (locks uploads)."""
    data = load_all()
...
# Stats
total, arrived, missing = get_counts()
st.markdown(f"""
<div class="stats-bar">
...
with st.expander("📋 Carregar lista de inscritas" if not locked else "🔒 Lista bloqueada (check-in iniciado)"):
    if locked:
        ...
        uploaded = st.file_uploader(
            "Envie .xlsx ou .csv (1 coluna com nomes)",
            type=["xlsx", "csv"],
        ...
        if uploaded:
            try:
                ...
                # skip header if it looks like text label
...
# Search
search = st.text_input("🔍 Buscar por nome ou sobrenome", placeholder="Digite para filtrar...", label_visibility="visible")
